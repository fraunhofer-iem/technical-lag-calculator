...
data class DependencyGraph(
    ...
    private fun calculateMetadata(): GraphMetadata {
        val numberOfStats = Node.numberOfStats(rootDependency)
    ...
    private fun linkDependencies(): Root {
        ...
        fun buildNodeWithChildren(node: ArtifactNode, visited: MutableSet<ArtifactNode>): LinkedArtifactDependencies {
            if (node in visited) {
                // Handle the cycle case here
                // For example, return a node with no children or throw an exception
                return LinkedArtifactDependencies(node, listOf())  // Returning node with no children in case of a cycle
            }
            ...
            return LinkedArtifactDependencies(node, children)
        }
